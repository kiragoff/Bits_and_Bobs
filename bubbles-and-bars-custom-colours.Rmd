...
---
Bubble plots and bar charts - custom colours
Kira Goff, 2025
...
Load the packages we're going to use. If R has been updated recently, you might need to re-install them. In that case, remove the # in front of "install" and then run the code chunk again.
```{r}
...
This version of the script is specifically for use when you are using a custom colour palette. 

Some resources: 
  - coolors.co/
  - colorhunt.co/
  - https://emilhvitfeldt.github.io/r-color-palettes/ (you can grab the hex codes from existing palettes as a base), https://redketchup.io/color-picker

Define your palette, using hex codes ("#2CA02CFF") or colour names ("darkgrey"). Make sure to provide as many colours as you need in the order you want them used. 
```{r}
colours = c
(
  "#1F77B4FF",  
...
If you only want to make bar charts, skip to the last code chunk. If you don't want to make a bar chart, don't run that code chunk.
...
If you have any relative abundances of zero, you will get a warning message that R removed x rows containing missing values. Yay! That warning means you're not putting a bubble if the organism isn't present. This is what we want. 

Bar charts! You've already loaded your csv file up top, so you can run this. 
